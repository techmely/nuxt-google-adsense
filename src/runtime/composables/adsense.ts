...
export function useAdsense(input: UseAdsenseOptions) {
  ...
  // computed properties
  const adRegion = computed(() =>
    show.value ? `page-${Math.random()}` : null
  );
  /**
  ...
  const isUnfilled = computed(
    () => ad.value?.getAttribute("data-ad-status") === "unfilled"
  );
  /**
  ...
  watch([path, queryParams], (newRoute, oldRoute) => {
    const routeChanged = hasRouteChanged(
      newRoute,
      oldRoute,
      options.includeQuery
    );

    if (ad.value && !ad.value.isConnected) return;
  ...
  watchEffect(() => {
    ...
    if (show.value) ad.value.innerHTML = "";
    else ad.value.innerHTML = " ";
  });
...
function useRouteAttr<K extends keyof RouteLocationNormalizedLoaded>(
  name: K
): ComputedRef<RouteLocationNormalizedLoaded[K]> {
...
function hasRouteChanged(
  ...
  return (
    newQueryKeys.length !== oldQueryKeys.length ||
